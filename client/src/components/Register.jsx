...
import { ShieldAlert, CheckCircle, User, Globe, Key, Lock, Shield, Loader2, ArrowRight, Sun, Moon, Eye, EyeOff, Heart } from 'lucide-react';
import { t } from '../translations';
import { syncUserToFirestore } from '../firebase';

export default function Register({ onSuccess, onRegisterOnly, onGoogleConnect, onSwitchToLogin, theme, onToggleTheme, language = 'en', onToggleLanguage }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!username || !email || !password) {
      setError(tr('auth.allFieldsRequired', 'All fields are required.'));
      setLoading(false);
      return;
    }
    if (password !== confirmPassword) {
      setError(tr('auth.passwordsDontMatch', 'Passwords do not match.'));
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || data.errors?.[0]?.msg || tr('auth.registrationFailed', 'Registration failed.'));
        setLoading(false);
        return;
      }

      setSuccess(tr('auth.accountCreatedSuccess', 'Account created successfully! Redirecting to login...'));
      setLoading(false);

      syncUserToFirestore(data.user || { username, email });

      setTimeout(() => {
        if (onSwitchToLogin) {
          onSwitchToLogin();
        } else if (onRegisterOnly) {
          onRegisterOnly();
        }
      }, 1500);
    } catch (err) {
      console.error(err);
      setError(tr('auth.networkError', 'Registration process failed to contact auth node.'));
      setLoading(false);
    }
  };
      ...
      <div className="relative w-full max-w-md bg-[var(--bg-panel)]/90 border border-[var(--border-main)] rounded-2xl p-8 backdrop-blur-md shadow-[0_0_50px_rgba(6,182,212,0.1)] flex flex-col gap-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-[10px] uppercase font-bold tracking-widest text-[var(--text-muted)] font-mono">{tr('auth.aliasLabel', 'Hacker Alias / Username')}</label>
            <div className="relative">
              <User className="absolute left-3.5 top-3 w-4 h-4 text-[var(--text-muted)]" />
              <input
                type="text"
                required
                placeholder={tr('auth.aliasPlaceholder', 'e.g. root_hacker_13')}
                value={username}
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] uppercase font-bold tracking-widest text-[var(--text-muted)] font-mono">{tr('auth.verifyPasswordLabel', 'Verify Encryption Password')}</label>
            <div className="relative flex items-center">
              ...
              <button
                ...
                className="absolute right-3.5 top-3 text-[var(--text-muted)] hover:text-[var(--text-bright)] transition-colors cursor-pointer"
                title={showConfirmPassword ? tr('auth.hidePassword', 'Hide password') : tr('auth.showPassword', 'Show password')}
              >
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <span>{tr('auth.registerNodeBtn', 'REGISTER PLATFORM NODE')}</span>
              </>
            )}
          </button>
        </form>
